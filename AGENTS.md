...
- `crates/publish`: 単一のpublisher crate。`vault` moduleをローカル入力境界、`render` moduleをcontent描画・Markdown変換・bookmark enrichmentの境界とし、分類、Obsidian link解決、artifact生成、orchestrationを内部moduleへ分ける。publisher専用処理をcrate外へ公開せず、外部APIはpublish entrypoint、bookmark enricher注入、`PublishError` / `Result`に絞る
