...
テストは `fixtures/site` の固定 artifact だけを読みます。private Obsidian submodule、S3、AWS credentials には依存しません。Playwright が `127.0.0.1:8008` でproduction Topcoat serverを起動し、home、about、category、article、404 status、metadata、client-side route遷移をChromiumで検証します。通常E2Eのserver buildはTopcoatのTailwind integrationとasset bundlerを使います。
