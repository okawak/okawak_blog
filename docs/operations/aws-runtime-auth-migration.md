...
- `terraform/`は参照専用とし、編集もcommand実行もしない
...
このrepositoryのTerraformにはrotation resourceが残るため、以降`terraform apply`を実行してはいけません。applyするとrotationが再有効化される可能性があります。
...
このphaseはAWS Consoleまたは管理用AWS CLIで行います。resource ARNはVPSのroot所有設定へ保存し、repositoryには環境固有値を追加しません。
...
TerraformはIAM user、initial access key、Secret、rotation Lambdaを宣言したままなので、AWS側撤去後は大きなdriftになります。このrepositoryのTerraformを再利用する場合は、事前に別Issueで次を決めます。

- read-only方針を解除してRoles Anywhere resourceをcode管理するか
- 別repository / 別IaCへownershipを移すか
- 既存Terraform stateから旧resourceを安全に外す方法

方針が決まるまで`terraform plan`や`terraform apply`を実行しません。
...
- Terraform driftと今後のIaC ownershipがIssueに記録されている
