...
Oracle Linux 9 x86-64へCloudflare公式RPM repositoryから導入します。OS全体は更新せず、`cloudflared`だけをinstallします。
...
Oracle Linux 9で確認済みのbinary pathは`/usr/local/bin/cloudflared`です。repository管理のsystemd unitもこのpathを使用します。
