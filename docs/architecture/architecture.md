...
IAM Roles Anywhereへの移行中、本番のAWS SDKは`AWS_SHARED_CREDENTIALS_FILE=/var/lib/okawak_blog/aws/credentials`を明示して使う。runtime credentialsはsystemdの`StateDirectory=okawak_blog`配下に置き、`ProtectHome=true`を維持したまま読み取れる構成とする。初回deploy前に、現在動作しているS3 reader profileを`service/bootstrap_aws_credentials.sh`で一度だけ複製する。これはrollback用のstatic credentialであり、定期更新やSecrets Manager読取は行わない。credential管理責務を`site/infra`へ持ち込まない。
