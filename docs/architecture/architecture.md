...
VPSのcredential反映は`okawak_blog-aws-credentials.timer`が日次で起動するoneshot serviceとする。Secrets Managerの値が現在のruntime fileと同じなら書き込みと再起動を行わない。変更時だけatomic replace、server再起動、`/api/ready`確認を行い、失敗時は旧credentialへ戻して再起動を試みた上でsystemdとjournalに失敗を残す。この処理は保存済みcredentialの反映であり、IAM access keyのrotationとは分離する。reader userが自分自身の旧keyで次のkeyを取得する構成は、旧key無効化後にbootstrapできないため採用しない。

長期的にはIAM Roles AnywhereのX.509 identityから`credential_process`でtemporary role credentialを得る構成へ移行し、static access keyと上記timerを撤去する。AWS側のresource準備、段階切替、rollback、certificate更新は[AWS runtime認証のIAM Roles Anywhere移行](../operations/aws-runtime-auth-migration.md)に定める。Codexを含む通常の実装では`terraform/`をread-onlyとするが、repository ownerが[Terraform変更計画](../operations/aws-runtime-auth-terraform-plan.md)に従ってRoles Anywhereとlegacy resource撤去をcode/stateに反映し、AWS実体とのdriftを残さない。
