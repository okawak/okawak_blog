...
現時点では publisher 側と reader 側の責務分離はまだ移行途中である。ただし publisher 専用の artifact 組み立てとローカル書き出しは `apps/publisher_artifacts` へ、Obsidian vault 走査と Markdown 変換は `apps/publisher_obsidian` へ移し始めており、この方向で `apps/` 側へ責務を寄せていく。そのため、既存コードへの過度な互換性維持よりも、今後の保守性・学習価値・実運用のしやすさを優先して再設計する。
...
### 3. publisher 側は `apps/`、reader 側は `crates/` に寄せる

このリポジトリでは、コード配置そのものも責務境界の一部として扱う。

publisher 側の以下の処理は `apps/` に置く。
...
一方で `crates/` は blog / Leptos サーバー側の責務に寄せる。

* `crates/domain`: publisher と reader が共有する純粋な契約とルール
* `crates/server`: S3 上の成果物を読むバックエンド
* `crates/web`: Leptos SSR UI
* `crates/infrastructure`: reader 側の S3 読み取り、キャッシュ、設定読込などの infrastructure

特に `crates/infrastructure` は publisher 用の置き場にはしない。Obsidian 読み取り、Markdown renderer、S3 upload をここへ入れないことを明示方針とする。
...
│   ├── infrastructure/    # reader 側の S3 / cache / runtime adapter
│   ├── server/            # 公開成果物を読む統合バックエンド
│   └── web/               # Leptos SSR 公開 UI
├── apps/
│   ├── obsidian_uploader/ # publisher CLI エントリポイント
...
* 既存の `obsidian_uploader` は publisher 側の中心アプリとして育てる
* `apps/publisher_artifacts` は publisher 側の artifact 組み立てと local writer を切り出す補助 crate として扱う
* `apps/publisher_obsidian` は Obsidian vault 走査、Front Matter 解析、Markdown 変換を切り出す補助 crate として扱う
* `crates/infrastructure` を導入または拡張する場合も、reader 側専用として扱う
* `shared` 的な箱は安易に導入しない
...
* `crates/infrastructure` を通して S3 の成果物を取得する
* トップ / カテゴリ / 記事ページを SSR する
* 必要に応じて静的ファイルを配信する

systemd service として起動する単一バイナリの中心はこちらに寄せる。

### `apps/obsidian_uploader` と publisher 側 crate
...
必要に応じて parser / renderer / uploader などの publisher 専用 crate を `apps/` 配下で分割する。
...
`obsidian_uploader` から切り出す publisher 専用ロジックの第一段階の受け皿として扱う。
...
`obsidian_uploader` から scan / parse / render 系の処理を切り出す受け皿として扱う。
...
### `crates/server` / `crates/infrastructure`
...
Java 的に抽象化を濫用するのではなく、必要な入出力だけを薄く trait に切り出す。reader 側の trait は `crates/`、publisher 側の trait は `apps/` で閉じることを優先する。
...
publisher 専用の Obsidian / Markdown / upload 実装を `crates/` 側へ増やさない。`crates/infrastructure` は reader 側専用とする。
...
### Phase 2: publisher 実装を `apps/` 配下で分割する

* parser / renderer / uploader などを必要に応じて `apps/` 配下の crate へ切り出す
* publisher 専用ロジックを `crates/` へ増やさない
* `apps/publisher_artifacts` と `apps/publisher_obsidian` を足がかりに publisher 専用ロジックを `apps/` 側へ整理する
...
### Phase 4: `server` / `web` / `infrastructure` を reader 側へ絞る

* S3 上の成果物を読む SSR サーバーにする
* 記事生成責務は持たせない
* `crates/infrastructure` は reader 側の adapter だけに絞る
...
2. `obsidian_uploader` を中心に publisher 側を `apps/` 配下で整理する段階的リファクタリング計画を立てること
3. `domain` を小さな型中心に再設計すること
4. publisher と reader の依存境界を崩さずに trait ベースの設計を提案すること
5. `crates/server` を単一バイナリの SSR サーバーとして成立させる設計案を示すこと
...
このリポジトリは、**Obsidian で書いた Markdown を `apps/` 配下の Rust 製 publisher が公開成果物へ変換し S3 に配置し、それを `crates/` 側の単一バイナリ Leptos SSR サーバーが nginx 配下で公開する、Rust らしいコンテンツ公開基盤**として再設計する。
