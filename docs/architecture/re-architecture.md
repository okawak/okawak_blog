...
現時点では publisher 側と reader 側の責務分離はまだ移行途中であり、特に `crates/application` には publisher 寄りの処理が暫定的に含まれている。そのため、既存コードへの過度な互換性維持よりも、今後の保守性・学習価値・実運用のしやすさを優先して再設計する。
...
* 現在存在する `crates/application` は移行途中の crate であり、publisher 専用コードの長期的な置き場としては増やさない
...
### `crates/application`（移行中）

現状は publisher 成果物生成の pure な組み立てが一部入っている移行途中の crate である。

ただし、publisher 専用ロジックの主置き場として増やし続ける前提では扱わない。
新しい publisher 専用の usecase や helper を追加する場合は、`apps/` 配下の crate として置く方針を優先する。

もし将来的に `crates/application` を残すなら、責務は以下のどちらかに限る。

* publisher / reader の両方で共有する pure な組み立て
* reader 側に閉じた小さな usecase
...
* 現在 `crates/application` に入っている publisher 寄りの補助ロジックは、長期的には `apps/` 側へ整理する
