...
現時点では publisher 側と reader 側の責務分離はまだ移行途中である。ただし publisher 専用の artifact 組み立てとローカル書き出しは `apps/publisher_artifacts` へ移し始めており、この方向で `apps/` 側へ責務を寄せていく。そのため、既存コードへの過度な互換性維持よりも、今後の保守性・学習価値・実運用のしやすさを優先して再設計する。
...
* `apps/publisher_artifacts` は publisher 側の artifact 組み立てと local writer を切り出す最初の補助 crate として扱う
...
* `apps/publisher_artifacts` を足がかりに publisher 専用ロジックを `apps/` 側へ整理する
