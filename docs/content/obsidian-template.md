...
この文書は、publishが処理できる Obsidian Markdown の最小テンプレートをまとめたものである。source of truth は private な Obsidian リポジトリ側にあり、この public リポジトリには通常ファイルとして commit しない。
...
本文の基本的な Markdown 記法は CommonMark に従う。Obsidian 固有の解釈だけで成立する強調記法などはpublishでは補正しない。
