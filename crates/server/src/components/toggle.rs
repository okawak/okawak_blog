// Vendored Topcoat UI components keep their full API while adoption is incremental.
#![allow(dead_code)]
...
const LINK: StaticClass = class!(
    "inline-flex shrink-0 items-center justify-center border border-transparent \
     font-medium whitespace-nowrap no-underline transition-colors select-none \
     focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 \
     focus-visible:ring-offset-background",
);

/// A navigation link styled as one member of a [`toggle_group`].
///
/// Use this when each choice changes the current document rather than a form
/// value. The ordinary anchor keeps navigation available without scripting.
#[component]
pub async fn toggle_link(
    active: bool,
    #[default] size: ToggleSize,
